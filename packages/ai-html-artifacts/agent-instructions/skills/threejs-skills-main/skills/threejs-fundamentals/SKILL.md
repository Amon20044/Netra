...
## Single-File Browser Setup (import map)

To run in the browser with no build step, load three.js from a versioned CDN via an
**import map** so bare `import … from "three"` specifiers resolve. `three/addons/` maps to
`examples/jsm/` (OrbitControls, loaders, post-processing, etc.). The consuming script MUST be
`type="module"`.
...
> Building a single-file **game**? See `threejs-games` for the loop, input, HUD and state
> machine. In artifact/sandbox contexts, keep this exact pinned import map — other hosts or
> floating versions may be rejected by the sanitizer.

### Animation loop: prefer `setAnimationLoop`

`renderer.setAnimationLoop(cb)` is preferred over a manual `requestAnimationFrame` loop: it
also drives WebXR and is cleanly stopped with `setAnimationLoop(null)`. The Quick Start uses
`requestAnimationFrame` for familiarity; both are valid.
