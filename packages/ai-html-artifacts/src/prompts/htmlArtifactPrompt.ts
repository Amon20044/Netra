...
- NEVER size by viewport HEIGHT (vh/svh/dvh/lvh) on big or full-bleed regions — heroes, banners, sections, "full screen" panels. Because the iframe's height equals the content height, a viewport-height value feeds back on itself and the page grows without bound (a hero "takes the whole screen" then balloons, spreading everything out). For a tall hero use a width- or pixel-based size instead, e.g. min-height:clamp(520px,60vw,820px) or an aspect-ratio box. Viewport WIDTH units (vw) are safe; only vh/svh/dvh are forbidden for region heights.
...
- Tap targets ≥ 44px. Do NOT use vh/svh/dvh for the height of heroes or sections (the content-sized iframe makes them feedback-loop and grow unbounded); size heroes by width/px instead (e.g. min-height:clamp(520px,60vw,820px)) and prefer content height everywhere else.
